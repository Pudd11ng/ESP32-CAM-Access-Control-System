...
void setup()
{
  ...
  Serial.println("Setting up relay pin...");
...
void handle_message(WebsocketsClient &client, WebsocketsMessage msg)
{
  ...
  if (msg.data().substring(0, 6) == "login:")
  { 

    // Initialize received credentials
    String receivedUsername;
    String receivedLoginPwd;

    // Find username
    int usernamePos = msg.data().indexOf("username=");
    if (usernamePos != -1)
    {
      usernamePos += 9; // Move past "username="
      int usernameEnd = msg.data().indexOf("&loginPwd=", usernamePos);
      if (usernameEnd != -1)
      {
        receivedUsername = msg.data().substring(usernamePos, usernameEnd);
      }
    }

    // Find loginPwd
    int loginPwdPos = msg.data().indexOf("loginPwd=");
    if (loginPwdPos != -1)
    {
      loginPwdPos += 9; // Move past "loginPwd="
      receivedLoginPwd = msg.data().substring(loginPwdPos);
    }

    // Compare received credentials with expected values
    if (receivedUsername.equals(username) && receivedLoginPwd.equals(loginPwd))
    {
